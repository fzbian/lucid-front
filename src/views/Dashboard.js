...
const DASHBOARD_ALLOWED_LOCALE_NAMES = ['Visto', 'Titanium', 'San Jose', 'Premium', 'Platinum', 'Medellin', 'San Fason'];

function isDashboardLocale(name) {
  const safe = String(name || '').trim().toLowerCase();
  return DASHBOARD_ALLOWED_LOCALE_NAMES.some((allowed) => {
    const base = allowed.toLowerCase();
    return safe === base || safe.startsWith(`${base} `) || safe.startsWith(`${base}(`);
  });
}
...
export default function Dashboard() {
  ...
  const localesList = useMemo(
    () => (caja?.locales
      ? Object.entries(caja.locales).filter(([nombre]) => isDashboardLocale(nombre)).sort(sortLocalesByName)
      : []),
    [caja]
  );
  const totalLocalesSaldo = localesList.reduce((acc, [, l]) => acc + (Number(l?.saldo_en_caja) || 0), 0);
  const totalLocalesVendido = localesList.reduce((acc, [, l]) => acc + (Number(l?.vendido) || 0), 0);
  const saldoTotalDashboard = (Number(caja?.saldo_caja) || 0) + (Number(caja?.saldo_caja2) || 0) + totalLocalesSaldo;
              ...
              <div>
                ...
                <p className="text-3xl font-extrabold leading-tight">
                  {showSaldoTotal ? (
                    <span
                      onClick={() => {
                        if (Number.isFinite(saldoTotalDashboard)) {
                          navigator.clipboard.writeText(Math.round(saldoTotalDashboard).toString());
                        }
                      }}
                      className="cursor-pointer hover:opacity-80 active:scale-95 transition-all inline-block"
                      title="Clic para copiar"
                    >
                      {formatCLP(saldoTotalDashboard)}
                    </span>
                  ) : '••••••'}
                </p>
              </div>
        ...
        <section className="bg-[var(--card-color)] rounded-2xl p-4 border border-[var(--border-color)] shadow">
            ...
            <div>
              ...
              {!loading && !checking && !error && (
                <p className="mt-1 text-xs text-[var(--text-secondary-color)]">
                  Mostrando {localesList.length} locales configurados
                </p>
              )}
            </div>
            <button
          ...
          {loading || checking ? (
            ...
            <div className="rounded-xl border border-dashed border-[var(--border-color)] bg-[var(--dark-color)]/40 px-4 py-6 text-center">
              ...
              <p className="mt-1 text-xs text-[var(--text-secondary-color)]">Solo se muestran: Visto, Titanium, San Jose, Premium, Platinum, Medellin y San Fason.</p>
            </div>
          ) : (
